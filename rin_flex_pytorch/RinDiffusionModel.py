...
class RinDiffusionModel(torch.nn.Module):
    ...
    @torch.no_grad()
    def sample(self, num_samples=64, target_size=None, iterations=100, method="ddim", seed=None, class_override=None):
        # If target_size is provided as (height, width), use those dimensions
        if target_size is None:
            target_size = (self.denoiser.image_shape[-2], self.denoiser.image_shape[-1])
        
        h, w = target_size
        # Ensure dimensions are divisible by patch_size
        h = ((h + self.denoiser._patch_size - 1) // self.denoiser._patch_size) * self.denoiser._patch_size
        w = ((w + self.denoiser._patch_size - 1) // self.denoiser._patch_size) * self.denoiser._patch_size
        
        samples_shape = [num_samples, self.denoiser._image_channels, h, w]
