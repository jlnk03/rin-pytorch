...
def create_model_from_config(config: dict, device: str = 'cuda'):
    ...
    # Build kwargs - sparse_hierarchy is optional (only in sparse-hier version)
    kwargs = dict(
    ...
    # Only add sparse_hierarchy if it exists in config (sparse-hier version only)
    if 'sparse_hierarchy' in rin_config:
        kwargs['sparse_hierarchy'] = rin_config['sparse_hierarchy']
...
def create_dummy_input(config: dict, batch_size: int = 1, device: str = 'cuda', use_tokens: bool = True):
    """Create dummy input for the model.
    
    Args:
        use_tokens: If True, create tokenized input (B, T, patch_dim). If False, create image input (B, C, H, W).
                   The rin-pytorch (padding) version expects tokenized input.
                   The rin-pytorch-sparse-hier version expects image input.
    """
    rin_config = config.get('rin', config.get('model', {}))
    ...
    # patch_dim = patch_size^2 * channels (raw pixel values per patch)
    ...
    if use_tokens:
        # Tokenized input for rin-pytorch (padding version)
        # x: (B, T, patch_dim) - flattened patch pixels
        x = torch.randn(batch_size, num_patches, patch_dim, device=device)
    else:
        # Image input for rin-pytorch-sparse-hier
        # x: (B, C, H, W) - raw image
...
def main():
    ...
    # Create dummy input
    # Detect if this is sparse-hier version (expects image input) or standard (expects tokenized input)
    rin_config = config.get('rin', config.get('model', {}))
    use_tokens = 'sparse_hierarchy' not in rin_config  # Standard rin-pytorch uses tokens
    inputs = create_dummy_input(config, args.batch_size, args.device, use_tokens=use_tokens)
